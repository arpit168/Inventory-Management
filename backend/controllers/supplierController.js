...
export const updateSupplier = async (req, res, next) => {
  try {
    const supplier = await Supplier.findOne({ _id: req.params.id, createdBy: req.user.id });
    if (!supplier) {
    ...
    await supplier.save();
    return res.status(200).json({ supplier, message: 'Supplier updated successfully' });
  } catch (error) {
    return next(error);
  }
};
...
export const deleteSupplier = async (req, res, next) => {
  try {
    const supplier = await Supplier.findOneAndDelete({ _id: req.params.id, createdBy: req.user.id });
    if (!supplier) {
      return res.status(404).json({ message: 'Supplier not found' });
    }
...
export const getSupplierLedger = async (req, res, next) => {
  try {
    const supplier = await Supplier.findOne({ _id: req.params.id, createdBy: req.user.id }).lean();
    if (!supplier) {
      return res.status(404).json({ message: 'Supplier not found' });
    }
...
const recalculateSupplierBalance = async (supplierId, userId) => {
  ...
  await Supplier.updateOne({ _id: supplierId, createdBy: userId }, { totalPayable, totalAdvance, netBalance });
};
...
export const addLedgerEntry = async (req, res, next) => {
  try {
    ...
    const entry = await SupplierLedgerEntry.create({
      ...
      type,
      amount: numAmount,
      description: description || (type === 'credit' ? 'Goods Received (We Owe)' : 'Payment Given'),
      date: date || new Date(),
      dueDate,
      createdBy: req.user.id,
    });
    ...
    return res.status(201).json({ entry, message: 'Transaction recorded successfully' });
  } catch (error) {
    return next(error);
  }
};
...
export const updateLedgerEntry = async (req, res, next) => {
  try {
    ...
    const entry = await SupplierLedgerEntry.findOne({ _id: req.params.entryId, supplier: req.params.id, createdBy: req.user.id });
...
export const deleteLedgerEntry = async (req, res, next) => {
  try {
    const entry = await SupplierLedgerEntry.findOneAndDelete({ _id: req.params.entryId, supplier: req.params.id, createdBy: req.user.id });
