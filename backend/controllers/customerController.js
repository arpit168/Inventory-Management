...
const sendLedgerNotificationEmail = async (
  userId,
  customer,
  type,
  amount,
  ...
  try {
    const profile = await BusinessProfile.findOne({ createdBy: userId }).sort({
      isDefault: -1,
      createdAt: -1,
    });
...
export const addLedgerEntry = async (req, res, next) => {
  try {
    ...
    const entry = await LedgerEntry.create({
      ...
      description:
        description ||
        (type === "credit" ? "Goods / Credit Given" : "Payment Received"),
      date: date || new Date(),
      dueDate,
      createdBy: req.user.id,
    });
    ...
    return res
      .status(201)
      .json({ entry, customer, message: "Transaction recorded successfully" });
  } catch (error) {
    return next(error);
  }
};
