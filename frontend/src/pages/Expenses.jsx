  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.25em] text-primary">Expense Analytics</p>
          <h1 className="text-2xl sm:text-3xl font-black text-text tracking-tight">Shop & Business Expenses</h1>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-wider font-bold text-text-muted">Total Recorded Expenses</p>
            <p className="mt-1 text-2xl font-black text-danger">₹{summary.totalAmount?.toFixed(2) || '0.00'}</p>
          </div>
        ...
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-wider font-bold text-text-muted">Highest Spending Category</p>
            <p className="mt-1 text-2xl font-black text-primary truncate">
              {topCat} <span className="text-sm font-bold text-text-muted">{topVal > 0 ? `(₹${topVal.toFixed(2)})` : ''}</span>
            </p>
          </div>
      ...
      <div className="rounded-2xl border border-border bg-surface shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="text-lg font-bold text-text">No Expenses Recorded</h3>
          ...
          <div className="w-full">
            ...
            <div className="hidden sm:block overflow-x-auto">
              ...
              <tbody className="divide-y divide-border">
                {expenses.map((exp) => (
                  <tr key={exp._id} className="hover:bg-background/50 transition-colors">
                    ...
                    <td className="py-4 px-5 text-text-muted font-medium flex items-center gap-1.5">
                      <Calendar size={14} className="text-text-muted" /> {new Date(exp.date).toLocaleDateString()}
                    </td>
                    <td className="py-4 px-5 font-black text-danger">₹{Number(exp.amount).toFixed(2)}</td>
          ...
          <div className="w-full max-w-md rounded-3xl border border-border bg-surface p-4 sm:p-6 shadow-2xl animate-scale-up my-8 sm:my-0">
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div>
                ...
                <h3 className="text-lg font-black text-text">Record New Expense</h3>
              </div>
              <button onClick={() => setIsAddOpen(false)} className="p-2 text-text-muted hover:bg-background hover:text-text rounded-xl transition">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateExpense} className="mt-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-text-muted mb-1.5">Expense Title *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-text-muted mb-1.5">Amount (₹) *</label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
              ...
              <div>
                <label className="block text-xs font-bold text-text-muted mb-1.5">Description / Notes</label>
                <input
                  type="text"
