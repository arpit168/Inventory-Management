...
const Dashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [analyticsResponse, , customersResponse, expensesResponse] =
          await Promise.all([
      ...
      <div className="rounded-3xl border border-border bg-surface  p-6 sm:p-8 text-white shadow-xl">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              to="/invoices"
              className="rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-slate-950 hover:bg-primary-hover transition shadow-sm"
            >
              View Invoices
            </Link>
            <Link
              to="/customers"
              className="rounded-xl bg-white/10 px-4 py-2.5 text-xs font-bold text-white hover:bg-white/20 transition backdrop-blur-xs"
            >
              Khatabook Ledger
            </Link>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl bg-white/5 p-5 border border-white/10 backdrop-blur-xs">
            <div className="flex items-center gap-3.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xl sm:text-2xl font-black text-danger mt-0.5 truncate">
                  ₹{financials.ledger.totalReceivable?.toFixed(2) || "0.00"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white/5 p-5 border border-white/10 backdrop-blur-xs">
            <div className="flex items-center gap-3.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xl sm:text-2xl font-black text-warning mt-0.5 truncate">
                  ₹{financials.expenses.totalAmount?.toFixed(2) || "0.00"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white/5 p-5 border border-white/10 backdrop-blur-xs">
            <div className="flex items-center gap-3.5">
              ...
              <div className="min-w-0">
                <p className="text-xs text-blue-500 font-semibold truncate">
                  Customer Advance Given
                </p>
                <p className="text-xl sm:text-2xl font-black text-success mt-0.5 truncate">
                  ₹{financials.ledger.totalAdvance?.toFixed(2) || "0.00"}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="rounded-2xl border border-border bg-surface p-6 shadow-xs">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-primary font-extrabold">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-success/20 bg-success/5 p-4">
              <p className="text-xs text-success font-bold uppercase tracking-wider">
                Total Profit
              </p>
        ...
        <div className="rounded-2xl border border-border bg-surface p-6 shadow-xs flex flex-col justify-between">
          ...
          <div className="space-y-2.5">
            {pieData.map((item, index) => (
              <div
                key={item.name}
                className="flex items-center justify-between rounded-xl border border-border bg-background px-4 py-2.5 text-xs font-semibold text-text"
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`h-2.5 w-2.5 rounded-full ${index === 0 ? "bg-primary" : "bg-danger"}`}
                  />
                  <span>{item.name}</span>
                </div>
                <span className="font-black">
                  {formatNumber(item.value)} items
                </span>
              </div>
            ))}
          </div>
            ...
            {(analytics.recentActivity || []).length === 0 ? (
              ...
              (analytics.recentActivity || []).map((item, index) => (
                <div
                  ...
                  className="flex items-center justify-between rounded-xl border border-border bg-background px-4 py-3 hover:border-primary/40 transition"
                >
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={`text-xs font-black ${item.quantityDelta > 0 ? "text-success" : "text-danger"}`}
                    >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-border bg-background p-3.5 text-center">
              ...
              <p className="mt-1 text-xl font-black text-warning">
                {analytics.totals.lowStockCount || 0}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-background p-3.5 text-center">
              <p className="text-xs font-bold text-text-muted uppercase">
              ...
              <p className="mt-1 text-xl font-black text-danger">
                {analytics.totals.outOfStockCount || 0}
              </p>
            </div>
          </div>
        </div>
