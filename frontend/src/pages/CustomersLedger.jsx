  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 md:px-6 max-w-full overflow-x-hidden animate-fade-in">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[10px] sm:text-xs font-extrabold uppercase tracking-[0.25em] text-blue-600 dark:text-blue-400">Ledger Suite</p>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-gray-900 dark:text-white tracking-tight">Khatabook Customer Ledger</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-5 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] sm:text-xs uppercase tracking-wider font-bold text-gray-500 dark:text-gray-400">Total Customers</p>
            <p className="mt-1 text-xl sm:text-2xl font-black text-gray-900 dark:text-white">{summary.totalCustomers || 0}</p>
          </div>
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-5 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] sm:text-xs uppercase tracking-wider font-bold text-gray-500 dark:text-gray-400">Total Due (Receivable)</p>
            <p className="mt-1 text-xl sm:text-2xl font-black text-red-600 dark:text-red-400">₹{(summary.totalReceivable || 0).toFixed(2)}</p>
          </div>
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-5 shadow-sm flex items-center justify-between sm:col-span-2 lg:col-span-1">
          <div>
            <p className="text-[10px] sm:text-xs uppercase tracking-wider font-bold text-gray-500 dark:text-gray-400">Total Advance Given</p>
            <p className="mt-1 text-xl sm:text-2xl font-black text-green-600 dark:text-green-400">₹{(summary.totalAdvance || 0).toFixed(2)}</p>
          </div>
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 sm:p-5 shadow-sm flex flex-col">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="relative flex-1 min-w-[120px]">
              <Search size={14} className="sm:w-4 sm:h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500" />
              <input
                type="text"
          ...
          <div className="space-y-2 flex-1 overflow-y-auto max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] pr-1">
            {loading ? (
              ...
              <p className="text-center text-xs sm:text-sm text-gray-500 dark:text-gray-400 py-12">No customers found.</p>
            ) : (
              customers.map((cust) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      <p className={`text-sm sm:text-base font-black ${isDue ? 'text-red-600 dark:text-red-400' : isAdvance ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 sm:p-5 shadow-sm flex flex-col min-h-[300px] sm:min-h-[400px] lg:min-h-[500px]">
          {!selectedCustomer ? (
            <div className="flex flex-1 flex-col items-center justify-center text-center p-8 sm:p-12">
              ...
              <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">Select a Customer</h3>
            ...
            <div className="flex flex-col flex-1">
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-gray-200 dark:border-gray-700 pb-3 sm:pb-4 mb-3 sm:mb-4">
                <div>
                  <h2 className="text-lg sm:text-xl font-black text-gray-900 dark:text-white">{selectedCustomer.name}</h2>
                  <div className="flex flex-wrap gap-2 sm:gap-3 text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">
                    <span>📞 {selectedCustomer.phone}</span>
                    {selectedCustomer.email && <span className="hidden sm:inline">✉️ {selectedCustomer.email}</span>}
                  </div>
                  {selectedCustomer.address && <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-1">📍 {selectedCustomer.address}</p>}
                </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-2 sm:space-y-3 pr-1 max-h-[200px] sm:max-h-[300px] lg:max-h-[400px]">
                {ledgerLoading ? (
                  ...
                  <p className="text-center text-xs sm:text-sm text-gray-500 dark:text-gray-400 py-12">No ledger entries recorded yet.</p>
                ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0 w-full sm:w-auto">
                          <div className={`rounded-xl p-1.5 sm:p-2.5 shrink-0 ${isCredit ? 'bg-red-500/15 text-red-600 dark:text-red-400' : 'bg-green-500/15 text-green-600 dark:text-green-400'}`}>
                            {isCredit ? <ArrowUpRight size={14} className="sm:w-[18px] sm:h-[18px]" /> : <ArrowDownLeft size={14} className="sm:w-[18px] sm:h-[18px]" />}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="text-xs sm:text-sm font-bold text-gray-900 dark:text-white truncate">{entry.description || (isCredit ? 'Credit Given' : 'Payment Received')}</p>
                            <p className="text-[10px] sm:text-[11px] text-gray-500 dark:text-gray-400">{new Date(entry.createdAt).toLocaleString()}</p>
                          </div>
                        </div>
                        ...
                        <div className="text-right shrink-0 w-full sm:w-auto flex flex-row sm:flex-col items-center justify-between sm:items-end gap-1 sm:gap-2">
                          <p className={`text-sm sm:text-base font-black ${isCredit ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                          ...
                          <div className="flex items-center gap-1 sm:gap-2">
                            ...
                            <button onClick={() => openEditEntry(entry)} className="text-[9px] sm:text-xs text-blue-600 dark:text-blue-400 hover:underline font-medium">Edit</button>
                            <button onClick={() => handleDeleteEntry(entry._id)} className="text-[9px] sm:text-xs text-red-600 dark:text-red-400 hover:underline font-medium">Del</button>
                          </div>
                        </div>
                      </div>
                    );
      ...
      {isAddCustomerOpen && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center bg-black/60 p-3 sm:p-4 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="w-full max-w-md rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-6 shadow-2xl animate-scale-up my-8 sm:my-0">
            ...
            <form onSubmit={handleCreateCustomer} className="mt-4 sm:mt-5 space-y-3 sm:space-y-4">
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Customer Name *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Phone Number *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Email (Optional)</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Address (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Opening Due Balance (₹)</label>
                <input
                  type="number"
                  step="0.01"
                  placeholder="0.00"
                  ...
                  className="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-3 sm:px-3.5 py-2 sm:py-2.5 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 outline-none transition"
                />
                <p className="text-[8px] sm:text-[10px] text-gray-500 dark:text-gray-400 mt-1">Enter positive for previous due, negative if advance received.</p>
              </div>
          ...
          <div className="w-full max-w-md rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-6 shadow-2xl animate-scale-up my-8 sm:my-0">
            <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-3 sm:pb-4">
              <div>
                ...
                <h3 className="text-base sm:text-lg font-black text-gray-900 dark:text-white">{selectedCustomer.name}</h3>
              </div>
              <button onClick={() => setIsEntryModalOpen(false)} className="rounded-xl p-1.5 sm:p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                <X size={16} className="sm:w-[18px] sm:h-[18px]" />
              </button>
            </div>

            <form onSubmit={handleAddOrEditEntry} className="mt-4 sm:mt-5 space-y-3 sm:space-y-4">
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1.5">Transaction Type</label>
              ...
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Amount (₹) *</label>
                <input
                  type="number"
                  min="0.01"
                  step="0.01"
              ...
              <div>
                <label className="block text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">Description / Note</label>
                <input
                  type="text"
